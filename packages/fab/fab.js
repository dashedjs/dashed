...
export class DashedFab extends DashedBase {
  ...
  template() {
    const templateFactory = (props = {}) => {
      const { borderRadius, dashWidth, dashLength, dashSpacing, dashColor, borderRadiusInner } = props;
      return html`
        ${sharedStyles}
        <style>
          :host {
    ...
    const { borderRadius = 24, dashWidth = 2, dashLength = 4, dashSpacing = 2, dashColor } = this.dashProps;
    const borderRadiusInner = borderRadius - 4;
    return templateFactory({ borderRadius, dashWidth, dashLength, dashSpacing, dashColor, borderRadiusInner });
  }
}
