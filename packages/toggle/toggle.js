...
export class DashedToggle extends DashedBase {
  ...
  template() {
    const templateFactory = (props = {}) => {
      const { borderRadius, dashWidth, dashLength, dashSpacing, dashColor, widthDelta, heightDelta } = props;
      return html`
        ${sharedStyles}
        <style>
          :host {
    ...
    const { borderRadius = 12, dashWidth = 2, dashLength = 8, dashSpacing = 2, dashColor } = this.dashProps;
    const [widthDelta, heightDelta] = [6, 10];
    return templateFactory({ borderRadius, dashWidth, dashLength, dashSpacing, dashColor, widthDelta, heightDelta });
  }
}
