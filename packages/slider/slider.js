...
export class DashedSlider extends DashedBase {
  ...
  template() {
    const templateFactory = (props = {}) => {
      const { borderRadius, dashWidth, dashLength, dashSpacing, dashColor, value, min, max, step, percentage } = props;
    ...
    const { borderRadius = 0, dashWidth = 2, dashLength = 2, dashSpacing = 1, dashColor } = this.dashProps;
    const { value = 30, min = 0, max = 100, step = 1 } = {
      value: parseFloat(this.value) || undefined,
      min: parseFloat(this.min) || undefined,
      max: parseFloat(this.max) || undefined,
      step: parseFloat(this.step) || undefined
    };
