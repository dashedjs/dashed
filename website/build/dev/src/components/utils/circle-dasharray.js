export function drawDashedCircle(svgCircle, circleProps, dashProps) {
  const {
    width,
    height
  } = circleProps;
  ...
  const {
    strokeDasharray,
    strokeDashOffset
  } = _calculateCircleStrokeDasharray(circleProps, validDashProps);

  _drawDash(svgCircle, strokeDasharray, strokeDashOffset);
}

function _drawCircle(svgCircle, width, height, strokeWidth) {
...
function _drawDash(svgCircle, strokeDasharray, strokeDashOffset) {
  svgCircle.setAttribute('stroke-dasharray', strokeDasharray);
  svgCircle.setAttribute('stroke-dashoffset', strokeDashOffset);
}

function _calculateCircleStrokeDasharray(hostProps, dashProps) {
  const {
    width,
    height
  } = hostProps;
  ...
  const {
    ...
    dashLength,
    dashRatio
  } = _validateLineDashProps(hostProps, dashProps);
  ...
  const dashSpacing = Math.round((circonference - dashCount * dashLength) / (dashCount + 1) * Math.pow(10, 3)) / Math.pow(10, 3);
