export function drawDashedRect(svgRect, hostProps, dashProps) {
  ...
  const {
    strokeDasharray,
    strokeDashOffset
  } = _calculateRectStrokeDasharray(hostProps, validDashProps);
...
function _calculateRectStrokeDasharray(hostProps, dashProps) {
  ...
  const {
    ...
    dashLength,
    dashRatio
  } = _validateRectDashProps(hostProps, dashProps);
  ...
  const dashCountX = _calculatePathDashCount(lineX, {
    dashLength,
    dashRatio
  });

  const dashCountY = _calculatePathDashCount(lineY, {
    dashLength,
    dashRatio
  });

  const dashCountCorner = _calculatePathDashCount(arcCorner, {
    dashLength,
    dashRatio
  });
  ...
  const strokeDashArrayCorner1 = _calculatePathStrokeDasharray(dashCountCorner, dashSpacingCorner, dashSpacingY, dashLength);

  const strokeDasharrayY = _calculatePathStrokeDasharray(dashCountY, dashSpacingY, dashSpacingCorner, dashLength);

  const strokeDashArrayCorner2 = _calculatePathStrokeDasharray(dashCountCorner, dashSpacingCorner, dashSpacingX, dashLength);
